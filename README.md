...
The notebook labeled Obtaining_Plant_village_data_works.ipynb in the Data folder was used to document the code that will enable us to retrieve the data we need for further analysis.
...
The ensuing cleaned and balanced dataset can be found in the Data folder with the name tomato_balanced1.

### Build CNN Model

In order to build the CNN model, all the images in the balanced dataset were first resized to 224x224 pixels. This is the common dimension for ResNet-50 model, VGG-19 models, and InceptionV3. The images were also preprocessed, shuffled and batched. This ensures that our data is properly optimized for building our models. The notebook DATA_BALANCED_CNN_and_SVM.ipynb in the Scripts folder contains all the code for each outlined section of model building below.

**ResNet-50 Model**:

The DATA_BALANCED_CNN_and_SVM.ipynb notebook in the Scripts folder contains the code for how to build this model and it is the first model to be built within this code script. The first step is to create empty list for our images and label: image_list and label_list. Then we used a for loop to convert and flatten our dataset into the image and label pairs they were in before, convert all of these images and labels into a Numpy array, and populate our image_list and label_list, respectively. These list were then transformed and saved into numpy arrays as images_np and labels_np. Next, this data was split into a 80% train and 20 temporary split. The 80% will be used to train the model while the 20% will be used to split for validation and test.
...
**VGG-19 Model**:
The DATA_BALANCED_CNN_and_SVM.ipynb notebook in the Scripts folder contains the code for how to build this model and it is the second model that is built and described within this code script.
...
**InceptionV3 Model**:
The DATA_BALANCED_CNN_and_SVM.ipynb notebook in the Scripts folder contains the code for how to build this model and it is the third model that is built and described within this code script.
...
### Build SVM Model
The same DATA_BALANCED_CNN_and_SVM.ipynb in the Scripts folder contains the code needed to build this model within the section labeled SVM modeling. It is the final model that is build and described within this script.
...
Code for the confusion matrices and classification reports and their ensuing outputs can be found within the DATA_BALANCED_CNN_and_SVM.ipynb notebook in the Scripts folder where under each section where a model was built, the corresponding confusion matrix and calssification report can be found for that specific model.
